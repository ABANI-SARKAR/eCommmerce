...
import com.abani.mart.databinding.ActivityCheckoutBinding;
...
public class CheckoutActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        totalPrice = (cart.getTotalPrice().doubleValue() * GST / 100) + cart.getTotalPrice().doubleValue();
        binding.total.setText("INR " + totalPrice);

        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        binding.checkoutBtn.setOnClickListener(view -> processOrder());
    }
    void processOrder() {
        progressDialog.show();
        ...
        try {
            ...
            for (Map.Entry<Item, Integer> item : cart.getAllItemsWithQty().entrySet()){
                ...
                productObj.put("price_item", productI.getPrice());
                productObj.put("product_id", productI.getId());
                productObj.put("product_name", productI.getName());
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, Api.POST_ORDER_URL, dataObject, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    if (response.getString("status").equals("success")) {
                        ...
                        new AlertDialog.Builder(CheckoutActivity.this).setTitle("Order Successful").setCancelable(false).setMessage("Your order number is: " + orderNumber)
                        ...
                        new AlertDialog.Builder(CheckoutActivity.this).setTitle("Order Failed").setMessage("Something went wrong, please try again.").setCancelable(false)
